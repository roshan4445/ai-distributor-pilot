...
//#region node_modules/.nitro/vite/services/ssr/assets/db-queries-D82C3Qnz.js
...
var supabaseUrl = typeof import.meta !== "undefined" && "https://jminggcexzicnakvsdlx.supabase.co" || typeof process !== "undefined" && process.env?.VITE_SUPABASE_URL || "https://jminggcexzicnakvsdlx.supabase.co";
var supabaseKey = typeof import.meta !== "undefined" && "sb_publishable_fRXfL-RO43PkxsuLF_vrLA_VEezLIta" || typeof process !== "undefined" && process.env?.VITE_SUPABASE_KEY || "sb_publishable_fRXfL-RO43PkxsuLF_vrLA_VEezLIta";
...
async function getMemory(conversationId) {
	try {
		const { data } = await supabase.from("messages").select("data").eq("id", `mem-${conversationId}`).maybeSingle();
		if (data?.data) {
...
async function updateMemory(conversationId, partial) {
...
Output a strict structured JSON matching the provided schema.`;
	...
	if (kind === "order" && data && data.items) await updateMemory(conversationId, { lastDraft: {
		items: data.items,
		total: data.total || 0
	} });
	if (confidence < .5) {
	...
	await updateMemory(conversationId, { recentConversation: [
...
async function runFallbackRulesEngine(text, products, dealers, invoices, orders, conversationId) {
	...
	if (lower.includes("repeat") || lower.includes("previous") || lower.includes("same")) {
		...
		await updateMemory(conversationId, { lastDraft: {
			items,
			total
		} });
