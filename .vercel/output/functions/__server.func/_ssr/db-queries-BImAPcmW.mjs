...
import { _ as HumanMessage, h as SystemMessage } from "../_libs/@langchain/core+[...].mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/distributorAgent-C1exxdAM.js
function __rest(s, e) {
	var t = {};
...
async function initDb() {
	...
	await db.execute(`DROP TABLE IF EXISTS dealers`);
	await db.execute(`DROP TABLE IF EXISTS conversation_state`);
	await db.execute(`DROP TABLE IF EXISTS dealer_profile`);
  ...
  `);
	console.log("Database initialized. Seeding mock data with UUIDs...");
	for (const d of [
		{
		...
		},
		{
			...
			name: "Mahesh Hardware & Electricals",
			city: "Tumakuru",
		...
		},
		{
			...
			name: "National Wires & Cables",
			city: "Mangaluru",
			phone: "+91 94800 78901",
			pending: 0,
			trust: 90,
			ordersCount: 15,
			lifetime: 98e4,
	...
	const seedOrders = [
		{
			...
			invoice: "INV-1042",
			dealerId: "00000000-0000-0000-0000-000000000003",
			dealerName: "Sri Lakshmi Agencies",
			total: 18240,
			status: "processing",
			placedAt: "2 min ago",
			created_at: (/* @__PURE__ */ new Date(Date.now() - 120 * 1e3)).toISOString(),
			aiNote: "Parsed from WhatsApp audio: 20x MCB 32A, 15x Modular Switch. Double-checked prices.",
			items: [{
				name: "MCB 32A Single Pole",
				qty: 20,
				price: 245
		...
		},
		{
			...
			invoice: "INV-1039",
			dealerId: "00000000-0000-0000-0000-000000000005",
			dealerName: "Mahesh Hardware & Electricals",
			total: 62400,
			status: "dispatched",
			placedAt: "2 days ago",
			created_at: (/* @__PURE__ */ new Date(Date.now() - 2880 * 60 * 1e3)).toISOString(),
			aiNote: "Held initially due to dues, released after partial payment promise.",
			items: [
				{
					name: "Copper Wire 2.5 sq mm (90m)",
					qty: 20,
	...
	for (const o of seedOrders) {
		await db.execute({
			sql: `INSERT INTO orders (id, invoice, dealerId, dealerName, total, status, placedAt, aiNote, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			args: [
				o.id,
				o.invoice,
		...
		for (const item of o.items) await db.execute({
	...
	await db.execute({
		sql: `INSERT INTO payment_promises (id, promised_date, promised_amount, dealer_id, status, created_at)
          VALUES (?, ?, ?, ?, ?, ?)`,
		args: [
			crypto.randomUUID(),
			"2026-11-05",
	...
	for (const inv of [
		...
		},
		{
			...
			invoice_code: "INV-1041",
			dealer: "ABC Electricals",
			amount: 33560,
			date: "Yesterday",
			status: "paid"
		},
		...
		},
		{
			...
			invoice_code: "INV-1039",
			dealer: "Mahesh Hardware & Electricals",
			amount: 62400,
			date: "4 days ago",
			status: "unpaid"
		},
		...
		sql: `INSERT INTO invoices (id, invoice_code, dealer, amount, date, status)
            VALUES (?, ?, ?, ?, ?, ?)`,
		args: [
			inv.id,
			inv.invoice_code,
	...
	const seedConversations = [
		{
			...
			city: "Mysuru",
			unread: 1,
			preview: "Sure sir, please dispatch by evening.",
			messages: [
				{
					id: "m1",
					fromRole: "dealer",
					time: "10:14 AM",
					text: "Sir, order 20 MCB (32A Single Pole) and 15 Modular Switches (6A). Standard delivery please."
				},
				{
					id: "m2",
					fromRole: "ai",
					...
					text: "Got it! I found both items in stock. Here is the order draft with wholesale pricing:"
				},
				{
					id: "m3",
					fromRole: "dealer",
				...
				},
				{
					...
					text: "Order confirmed and invoice generated. Stock has been deducted."
				},
				{
					id: "m5",
					fromRole: "dealer",
					time: "10:20 AM",
					text: "Sure sir, please dispatch by evening."
				}
			]
		},
		{
			...
			city: "Bengaluru",
			unread: 0,
			preview: "Paid ₹20,000 today via UPI. Remaining after Diwali sir.",
			messages: [
				{
					id: "m6",
					fromRole: "dealer",
					time: "Yesterday",
					text: "Sir, overdue invoice of ₹76,000 details please."
				},
				{
					id: "m7",
					fromRole: "ai",
					...
					text: "Here is the current dues report for Raj Traders. Total outstanding: ₹1,24,500."
				},
				{
					id: "m8",
					fromRole: "dealer",
					time: "9:30 AM",
					text: "Paid ₹20,000 today via UPI. Remaining after Diwali sir."
				},
				{
					id: "m9",
					fromRole: "ai",
					...
					text: "Thank you! Received ₹20,000. Outstanding balance has been updated in the ledger:"
				},
				{
					id: "m10",
					fromRole: "ai",
					...
					data: JSON.stringify({
						when: "Nov 5, 10:00 AM",
						note: "Gentle payment nudge for Raj Traders (₹1,04,500)"
					}),
					text: "I have scheduled a follow-up reminder for the remaining balance."
				}
			]
		},
		{
			...
			city: "Bengaluru",
			unread: 0,
			preview: "Invoice received, making payment.",
			messages: [
				{
					id: "m11",
					fromRole: "dealer",
					time: "2 days ago",
					text: "Send last invoice copy."
				},
				{
					id: "m12",
					fromRole: "ai",
					...
					text: "Here is the generated invoice copy for ABC Electricals:"
				},
				{
					id: "m13",
					fromRole: "dealer",
					time: "Yesterday",
					text: "Invoice received, making payment."
				}
			]
		}
	];
	for (const c of seedConversations) {
		...
		for (const m of c.messages) await db.execute({
	...
	await db.execute(`
    INSERT INTO dealer_profile (dealer_id, trust_score, preferred_language)
    VALUES 
      ('00000000-0000-0000-0000-000000000001', 62, 'mixed'),
      ('00000000-0000-0000-0000-000000000002', 94, 'en'),
      ('00000000-0000-0000-0000-000000000003', 88, 'mixed')
  `);
}
var supabaseUrl = typeof process !== "undefined" && process.env?.VITE_SUPABASE_URL || "https://jminggcexzicnakvsdlx.supabase.co";
var supabaseKey = typeof process !== "undefined" && process.env?.VITE_SUPABASE_KEY || "sb_publishable_fRXfL-RO43PkxsuLF_vrLA_VEezLIta";
var realSupabase = createClient$1(supabaseUrl, supabaseKey);
var SqliteQueryBuilder = class {
	table;
	action = "select";
	selectCols = "*";
	insertData = null;
	...
	async then(onfulfilled) {
		try {
			...
			if (this.action === "select") {
				...
				if (this.wheres.length > 0) sql += " WHERE " + this.wheres.map((w) => {
					args.push(w.val);
					return `"${w.col}" ${w.op} ?`;
				}).join(" AND ");
				if (this.orderByCol) sql += ` ORDER BY "${this.orderByCol}" ${this.orderDesc ? "DESC" : "ASC"}`;
				...
				for (const row of dataArr) {
					...
					const rowVals = Object.values(row).map((v) => v === void 0 || v === null ? null : typeof v === "object" ? JSON.stringify(v) : v);
					const placeholders = rowKeys.map(() => "?").join(", ");
					const insertSql = `INSERT INTO "${this.table}" (${rowKeys.map((k) => `"${k}"`).join(", ")}) VALUES (${placeholders})`;
					await db.execute({
						sql: insertSql,
						args: rowVals
					});
				...
				const updates = Object.keys(this.updateData).map((k) => {
					let val = this.updateData[k];
					if (val === void 0 || val === null) val = null;
			...
			if (this.isSingle) data = data.length > 0 ? data[0] : null;
			const result = {
				data,
				error: null
			};
			...
			console.error(`SqliteQueryBuilder error on ${this.table}:`, err);
			const result = {
				data: null,
				error: err
			};
...
var supabase = typeof process !== "undefined" && process.env?.USE_SQLITE === "true" ? {
...
console.log("Supabase Client Init: URL =", supabaseUrl || "MISSING", "KEY =", supabaseKey ? "PRESENT" : "MISSING", "SQLITE MODE =", typeof process !== "undefined" && process.env?.USE_SQLITE === "true");
...
function logAgentExecution(log) {
	...
	if (log.totalTokens !== void 0) console.log(`🪙 Tokens Used:     Prompt: ${log.promptTokens} | Response: ${log.responseTokens} | Total: ${log.totalTokens}`);
	if (log.tokenSavingsLog) console.log(`💡 Token Savings:   ${log.tokenSavingsLog}`);
	...
	ORDER_DRAFT: [
		{
			type: "tool",
			name: "verifySKU",
			purpose: "Resolve SKU from catalog"
		},
		{
			type: "tool",
			name: "resolvePrice",
			purpose: "ALWAYS overwrite LLM price with live DB catalog price"
		},
		{
			type: "tool",
			name: "checkInventory",
			purpose: "Check stock availability at draft time"
		}
	],
	ORDER_CONFIRM: [
		{
			type: "tool",
			name: "verifySKU",
			purpose: "Re-verify SKU"
		},
		{
			type: "tool",
			name: "resolvePrice",
			purpose: "Re-confirm live DB price at commit time"
		},
		{
			type: "tool",
			name: "checkInventory",
		...
		},
		{
			type: "tool",
			name: "deductStock",
			purpose: "Deduct inventory"
		},
		{
			type: "tool",
			name: "createOrderRecord",
			purpose: "Create order + items records"
		},
		{
			type: "tool",
		...
		},
		{
			type: "tool",
			name: "updateDues",
			purpose: "Update dealer dues ledger"
		}
	],
	REPEAT_ORDER: [
		{
			type: "tool",
			name: "fetchLastOrder",
			purpose: "Query orders/order_items table for this dealer_id, most recent by created_at"
		},
		{
			type: "tool",
			name: "resolvePrice",
			purpose: "Re-price all items at CURRENT catalog prices, never reuse stored historical prices"
		},
		{
			type: "tool",
			name: "checkInventory",
	...
	PAYMENT: [
		{
			type: "tool",
			name: "recordPayment",
			purpose: "Register payment"
		},
		{
			type: "tool",
			name: "updateLedger",
			purpose: "Update dues balance"
		},
		{
			type: "tool",
			name: "recalculateTrustScore",
			purpose: "Update dealer_profile.trust_score"
		}
	],
	PAYMENT_PROMISE: [{
		type: "tool",
		name: "scheduleReminder",
		purpose: "Schedule follow-up reminder"
	}, {
		type: "tool",
		name: "updateDuesRecord",
		purpose: "Flag promised payment date"
	}],
	BUSINESS_QUERY: [{
		type: "llm",
		name: "queryBusiness",
		purpose: "Answer natural language query using live data"
	}]
};
function getPlan(intent, isDraftOnly) {
...
async function resolvePrices(items) {
	...
	for (const item of items) {
		const { data: product } = await supabase.from("products").select("price, name, sku").eq("sku", item.sku).maybeSingle();
		if (!product) {
			const { data: fallbackProduct } = await supabase.from("products").select("price, name, sku").ilike("sku", item.sku).limit(1).maybeSingle();
			const prod = fallbackProduct || product;
			if (prod) resolved.push({
				sku: prod.sku,
				name: prod.name,
				qty: item.qty,
				price: prod.price
...
async function getConversationState(conversationId, dealerId) {
	try {
		const { data, error } = await supabase.from("conversation_state").select("*").eq("conversation_id", conversationId).maybeSingle();
		if (error) console.error("Error fetching conversation state:", error);
		if (data) return data;
		const newState = {
		...
		const { data: inserted, error: insertError } = await supabase.from("conversation_state").insert(newState).select().maybeSingle();
		if (insertError) console.error("Error inserting conversation state:", insertError);
		return inserted || newState;
	} catch (e) {
...
async function updateConversationState(conversationId, updates) {
	try {
		...
		await supabase.from("conversation_state").update(cleanUpdates).eq("conversation_id", conversationId);
	} catch (e) {
...
async function getDealerProfile(dealerId) {
	try {
		const { data, error } = await supabase.from("dealer_profile").select("*").eq("dealer_id", dealerId).maybeSingle();
		if (error) console.error("Error fetching dealer profile:", error);
		if (data) return data;
		const newProfile = {
			dealer_id: dealerId,
			trust_score: 100,
		...
		const { data: inserted, error: insertError } = await supabase.from("dealer_profile").insert(newProfile).select().maybeSingle();
		if (insertError) console.error("Error inserting dealer profile:", insertError);
...
async function resolveDealerId(conversationId, dealerName) {
	if (conversationId === "ask-ai-convo" || dealerName === "Business Owner") {
		...
		return firstDealer?.id || "00000000-0000-0000-0000-000000000001";
	}
	if (dealerName && dealerName !== "Unknown" && dealerName !== "Business Owner") {
		const { data: activeDealer } = await supabase.from("dealers").select("id").eq("name", dealerName).maybeSingle();
		if (activeDealer?.id) return activeDealer.id;
	}
	const { data: convo } = await supabase.from("conversations").select("dealer").eq("id", conversationId).maybeSingle();
	if (convo?.dealer) {
		const { data: activeDealer } = await supabase.from("dealers").select("id").eq("name", convo.dealer).maybeSingle();
		if (activeDealer?.id) return activeDealer.id;
	}
	const { data: firstDealer } = await supabase.from("dealers").select("id").limit(1).maybeSingle();
	return firstDealer?.id || "00000000-0000-0000-0000-000000000001";
}
function calculateDueDate(whenStr) {
	try {
		if (!whenStr) return new Date(Date.now() + 10080 * 60 * 1e3).toISOString();
		const clean = whenStr.toLowerCase();
		if (clean.includes("tomorrow")) return new Date(Date.now() + 1440 * 60 * 1e3).toISOString();
		if (clean.includes("diwali") || clean.includes("nov 5")) return (/* @__PURE__ */ new Date("2026-11-05T10:00:00.000Z")).toISOString();
		if (clean.includes("dussehra") || clean.includes("dusheera")) return (/* @__PURE__ */ new Date("2026-10-20T10:00:00.000Z")).toISOString();
		const parsed = Date.parse(whenStr);
		if (!isNaN(parsed)) return new Date(parsed).toISOString();
		return new Date(Date.now() + 10080 * 60 * 1e3).toISOString();
	} catch (err) {
		return new Date(Date.now() + 10080 * 60 * 1e3).toISOString();
	}
}
async function invokeWithRetry(fn, maxRetries = 2, delay = 1e3) {
	let attempt = 0;
	while (true) try {
		return await fn();
	} catch (e) {
		...
		console.warn(`Groq retry attempt ${attempt} due to: ${e.message}`);
		await new Promise((res) => setTimeout(res, delay * attempt));
	}
}
async function classifyIntent(message, lastDraft, lastIntent) {
	const apiKey = process.env.GROQ_API_KEY;
	if (!apiKey) {
		console.warn("⚠️ GROQ_API_KEY is missing from environment variables! Running fallback classifier.");
		return runFallbackClassifier(message, lastDraft, lastIntent);
	}
	const systemPrompt = `You are the core AI Classifier for Kumar Electricals & Distribution.
Analyze the dealer's input message and classify its intent.

CURRENT CONTEXT:
- lastDraft: ${JSON.stringify(lastDraft)}
- lastIntent: ${lastIntent || "None"}

RULES:
1. Support mixed language (Hindi/Telugu/English) natively (Roman transliterated script included). Do NOT translate first — extract intent/entities directly.
2. Output a confidence score between 0 and 1.
3. Detect "subIntent" for ORDER:
   - 'new_draft': if the user wants to start a new order draft, or discard the old draft and start a new one.
   - 'correction': if there is an active draft (lastDraft is not null) and they want to correct it (add, remove, change quantities/items).
   - 'confirm': if there is an active draft and they want to confirm/finalize it.
   - null: for other intents.

FEW-SHOT EXAMPLES:
1. Message: "Sir, order 20 MCB (32A Single Pole) and 15 Modular Switches (6A)."
   Output: {"intent": "ORDER", "confidence": 0.98, "subIntent": "new_draft", "extractedItems": [{"sku": "MCB-32A-SP", "qty": 20}, {"sku": "SW-MOD-6A", "qty": 15}]}
2. Message: "wo Havells wala switch 20 pieces de do"
   Output: {"intent": "ORDER", "confidence": 0.90, "subIntent": "new_draft", "extractedItems": [{"sku": "SW-MOD-6A", "qty": 20}]}
3. Message: "Bhai quantity change kar do, modular switch 10 pieces baje aur panel light add karo 5 pieces"
   Output: {"intent": "ORDER", "confidence": 0.95, "subIntent": "correction", "extractedChanges": [{"sku": "SW-MOD-6A", "qty": 10, "action": "update"}, {"sku": "LED-PL-12W", "qty": 5, "action": "add"}]}
4. Message: "Confirm order please sir"
   Output: {"intent": "ORDER", "confidence": 0.99, "subIntent": "confirm"}
5. Message: "Paid ₹20,000 today via UPI. Remaining after Diwali sir."
   Output: {"intent": "PAYMENT", "confidence": 0.98, "subIntent": null, "paymentAmount": 20000}
6. Message: "Diwali ke baad outstanding balance bhej dunga"
   Output: {"intent": "PAYMENT_PROMISE", "confidence": 0.94, "subIntent": null, "reminderWhen": "Nov 5, 10:00 AM", "reminderNote": "Outstanding dues follow-up"}
7. Message: "Total ledger status aur pending dues batao"
   Output: {"intent": "BUSINESS_QUERY", "confidence": 0.96, "subIntent": null}
8. Message: "LED Light panel stock bacha hai kya?"
   Output: {"intent": "PRODUCT_QUERY", "confidence": 0.95, "subIntent": null}
9. Message: "same as last order"
   Output: {"intent": "REPEAT_ORDER", "confidence": 0.98, "subIntent": null}
10. Message: "sir repeat my usual order"
    Output: {"intent": "REPEAT_ORDER", "confidence": 0.96, "subIntent": null}

OUTPUT FORMAT:
Return a strict JSON object with:
- "intent": "ORDER" | "PAYMENT" | "PAYMENT_PROMISE" | "INVOICE" | "PRODUCT_QUERY" | "BUSINESS_QUERY" | "REPEAT_ORDER"
- "confidence": number (0 to 1)
- "subIntent": "new_draft" | "correction" | "confirm" | null
- "extractedItems": [{"sku": string, "qty": number}]
- "extractedChanges": [{"sku": string, "qty": number, "action": "add"|"remove"|"update"}]
- "paymentAmount": number | null
- "reminderWhen": string | null
- "reminderNote": string | null
- "response": string (polite manager response, address as sir)`;
	const callLLM = async () => {
		...
		const text = (await llm.invoke([new SystemMessage(systemPrompt), new HumanMessage(userMessage)])).content.toString().trim();
		return JSON.parse(text);
	};
	try {
		return await invokeWithRetry(callLLM);
	} catch (err) {
		console.error("Groq classifier invocation failed, using offline fallback:", err);
		return runFallbackClassifier(message, lastDraft, lastIntent);
	}
}
function runFallbackClassifier(message, lastDraft, lastIntent) {
	const lower = message.toLowerCase();
	if (lastDraft && (lower.includes("correct") || lower.includes("change") || lower.includes("update") || lower.includes("modify") || lower.includes("cancel") || lower.includes("instead") || lower.includes("baje") || lower.includes("quantity") || lower.includes("pieces") || lower.includes("piece") || lower.includes("make the") || lower.includes("make") || lower.includes("give me") || lower.includes("ok give") || lower.includes("then") || lower.includes("instead of"))) {
		const qtyMatch = message.match(/\b\d+\b/);
		const qty = qtyMatch ? Number(qtyMatch[0]) : 20;
		let sku = "MCB-32A-SP";
		const mcbIdx = lower.indexOf("mcb");
		const swIdx = lower.indexOf("switch");
		const wireIdx = lower.indexOf("wire");
		if (swIdx > -1 && (mcbIdx === -1 || swIdx < mcbIdx) && (wireIdx === -1 || swIdx < wireIdx)) sku = "SW-MOD-6A";
		else if (wireIdx > -1 && (mcbIdx === -1 || wireIdx < mcbIdx) && (swIdx === -1 || wireIdx < swIdx)) sku = "WIRE-1.5CU";
		else {
			sku = "MCB-32A-SP";
			if (lower.includes("12a")) sku = "MCB-12A-SP";
			else if (lower.includes("16a")) sku = "MCB-16A-SP";
		}
		const action = lower.includes("cancel") || lower.includes("remove") ? "remove" : "update";
		return {
			intent: "ORDER",
			confidence: .9,
			...
			response: "Understood sir! I am updating your order draft."
		};
	}
	if (lower.includes("confirm") || lower.includes("yes") || lower.includes("agree") || lower.includes("done") || lower.includes("ok") || lower.includes("okay") || lower.includes("theek") || lower.includes("place")) return {
		intent: "ORDER",
		confidence: .95,
		subIntent: "confirm",
		response: "Outstanding sir! I have registered your order confirmation."
	};
	if (lower.includes("same as last") || lower.includes("repeat my usual") || lower.includes("repeat order") || lower.includes("usual order") || lower.includes("repeat last")) return {
		intent: "REPEAT_ORDER",
		confidence: .98,
		subIntent: null,
		response: "Fetching your last order details..."
	};
	if (lower.includes("order") || lower.includes("need") || lower.includes("want") || lower.includes("buy") || lower.includes("send") || lower.includes("mcb") || lower.includes("switch") || lower.includes("wire") || lower.includes("light") || lower.includes("de do") || lower.includes("de-do")) {
		const extractedItems = [];
		const segments = lower.split(/and|,|aur/);
		for (const seg of segments) {
			const qtyMatch = seg.match(/\d+/);
			const qty = qtyMatch ? Number(qtyMatch[0]) : 10;
			let sku = "";
			if (seg.includes("mcb-32a-sp") || seg.includes("mcb") && seg.includes("32a")) sku = "MCB-32A-SP";
			else if (seg.includes("mcb-12a-sp") || seg.includes("mcb") && seg.includes("12a")) sku = "MCB-12A-SP";
			else if (seg.includes("mcb-16a-sp") || seg.includes("mcb") && seg.includes("16a")) sku = "MCB-16A-SP";
			else if (seg.includes("sw-mod-6a") || seg.includes("switch")) sku = "SW-MOD-6A";
			else if (seg.includes("wire-1.5cu") || seg.includes("wire")) sku = "WIRE-1.5CU";
			else if (seg.includes("mcb")) sku = "MCBs";
			if (sku) extractedItems.push({
				sku,
				qty
			});
		}
		if (extractedItems.length === 0) extractedItems.push({
			sku: "MCB-32A-SP",
			qty: 10
		});
		return {
			...
			subIntent: "new_draft",
			extractedItems,
			response: "Drafting order..."
		};
	}
	if (lower.includes("paid") || lower.includes("remitted") || lower.includes("transferred") || lower.includes("sent")) {
		...
		return {
			...
			response: `Thank you sir! Received ₹${amount.toLocaleString("en-IN")}.`
		};
	}
	if (lower.includes("diwali") || lower.includes("dusheera") || lower.includes("promise") || lower.includes("will pay") || lower.includes("pay later") || lower.includes("pay after") || lower.includes("bhej dunga") || lower.includes("de dunga")) {
		const match = message.match(/\d+/);
		let amount = match ? Number(match[0]) : null;
		if (amount && (lower.includes("nov " + amount) || lower.includes("oct " + amount) || amount < 100)) amount = null;
		let reminderWhen = "Nov 5, 10:00 AM";
		if (lower.includes("tomorrow")) reminderWhen = "tomorrow";
		return {
			intent: "PAYMENT_PROMISE",
			confidence: .95,
			paymentAmount: amount,
			reminderWhen,
			reminderNote: "Payment promise dues nudge",
			response: "Understood sir. I have registered your payment promise."
		};
	}
	return {
		intent: "BUSINESS_QUERY",
		confidence: .85,
		response: "Hello sir, please let me know how I can help you check stock availability or confirm orders."
	};
}
async function recalculateDraft(lastDraft, extractedChanges) {
	let items = [...lastDraft.items || []];
	for (const change of extractedChanges || []) {
		const sku = change.sku;
		const qty = Number(change.qty);
		const action = change.action || "update";
		const index = items.findIndex((it) => it.sku.toLowerCase() === sku.toLowerCase());
		if (action === "remove" || qty === 0) {
			if (index > -1) items.splice(index, 1);
		} else if (action === "add") if (index > -1) items[index].qty += qty;
		else {
			const { data: prod } = await supabase.from("products").select("name, price, sku").eq("sku", sku).maybeSingle();
			items.push({
				sku,
				name: prod?.name || sku,
				qty,
		...
		else if (index > -1) items[index].qty = qty;
		else {
			const { data: prod } = await supabase.from("products").select("name, price, sku").eq("sku", sku).maybeSingle();
			items.push({
				sku,
	...
	items = await resolvePrices(items);
...
function checkStaleness(lastActivityAt, turnCount, lastIntent) {
	const lastActive = new Date(lastActivityAt).getTime();
	if ((Date.now() - lastActive) / (1e3 * 60) > 10) return true;
	if (turnCount > 3) return true;
	if (lastIntent && lastIntent !== "ORDER" && lastIntent !== "REPEAT_ORDER") return true;
	return false;
}
async function reflectAndVerify(result) {
	if (!result || !result.invoiceId) return {
		consistent: false,
		message: "No invoice ID generated"
	};
	try {
		const { data: invoice } = await supabase.from("invoices").select("*").eq("invoice_code", result.invoiceId).maybeSingle();
		if (!invoice) return {
			consistent: false,
			message: "Invoice record not found in Supabase database."
		};
		if (Math.abs(invoice.amount - result.total) > .01) return {
			consistent: false,
			message: `Price mismatch in Supabase: expected ${result.total}, got ${invoice.amount}`
		};
		return {
			consistent: true,
			message: "Verification success: database write matches."
		};
	} catch (e) {
		return {
			consistent: false,
			message: `DB Consistency check failed: ${String(e)}`
		};
	}
}
function parsePromisedDate(when) {
	if (!when) {
	...
	if (lower.includes("nov 5") || lower.includes("post-diwali")) return "2026-11-05";
...
async function adjustTrustScore(dealerId) {
	const baseline = {
		"00000000-0000-0000-0000-000000000001": 62,
		"00000000-0000-0000-0000-000000000002": 94,
		"00000000-0000-0000-0000-000000000003": 88,
		"00000000-0000-0000-0000-000000000004": 74,
		"00000000-0000-0000-0000-000000000005": 55,
		"00000000-0000-0000-0000-000000000006": 85,
		"00000000-0000-0000-0000-000000000007": 90
	}[dealerId] || 90;
	const { data: promises } = await supabase.from("payment_promises").select("status").eq("dealer_id", dealerId);
	const keptCount = (promises || []).filter((p) => p.status === "kept").length;
	const brokenCount = (promises || []).filter((p) => p.status === "broken").length;
	let newScore = baseline + keptCount * 3 - brokenCount * 8;
	newScore = Math.max(30, Math.min(100, newScore));
	await supabase.from("dealers").update({ trust: newScore }).eq("id", dealerId);
	await supabase.from("dealer_profile").update({ trust_score: newScore }).eq("dealer_id", dealerId);
	return newScore;
}
async function executePlan(plan, context, lastDraft) {
	let items = lastDraft ? [...lastDraft.items || []] : [];
	if (context.items) if (!lastDraft) items = [...context.items];
	else for (const item of context.items) {
		const idx = items.findIndex((it) => it.sku.toLowerCase() === item.sku.toLowerCase());
		if (idx > -1) items[idx].qty = item.qty;
		else items.push(item);
	}
	let total = lastDraft ? lastDraft.total : 0;
	let invoiceId = "";
	for (const step of plan) if (step.type === "tool") {
		if (step.name === "verifySKU") {
			const { data: dbProducts } = await supabase.from("products").select("*");
			const products = dbProducts || [];
			const ambiguousMatches = [];
			const validatedItems = [];
			for (const item of items) {
				const queryText = (item.sku || item.name || "").toLowerCase();
				let matches = products.filter((p) => p.sku.toLowerCase() === queryText || p.name.toLowerCase() === queryText || p.sku.toLowerCase().includes(queryText) || p.name.toLowerCase().includes(queryText));
				if (matches.length === 0) matches = products.filter((p) => queryText.split(" ").every((word) => p.name.toLowerCase().includes(word)));
				if (matches.length > 1) {
					const exactSku = matches.find((p) => p.sku.toLowerCase() === queryText);
					if (exactSku) validatedItems.push({
						sku: exactSku.sku,
						name: exactSku.name,
						qty: Number(item.qty) || 1,
						price: Number(item.price || exactSku.price)
					});
					else ambiguousMatches.push({
						item,
						matches
					});
					...
					validatedItems.push({
						...
						name: match.name,
						qty: Number(item.qty) || 1,
						price: Number(item.price || match.price)
					});
				} else throw new Error(`SKU/Product matching "${item.sku || item.name}" not found in catalog.`);
			}
			if (ambiguousMatches.length > 0) throw {
				type: "AMBIGUOUS_SKU",
				ambiguousMatches,
				validatedItems
			};
		...
		if (step.name === "resolvePrice") {
			items = await resolvePrices(items);
			total = items.reduce((sum, it) => sum + it.qty * it.price, 0);
		}
		if (step.name === "checkInventory") {
			const { data: dbProducts } = await supabase.from("products").select("*");
			const products = dbProducts || [];
			const outOfStockItems = [];
			const inStockItems = [];
			for (const item of items) {
				const match = products.find((p) => p.sku === item.sku);
				if (match && match.stock < item.qty) outOfStockItems.push({
					item,
					available: match.stock
				});
				else inStockItems.push(item);
			}
			if (outOfStockItems.length > 0) {
				for (const oos of outOfStockItems) await supabase.from("stock_alerts").insert({
					id: crypto.randomUUID(),
					sku: oos.item.sku,
					product_name: oos.item.name,
					requested_qty: oos.item.qty,
					available_stock: oos.available,
		...
		if (step.name === "deductStock") for (const item of items) {
		...
		if (step.name === "createOrderRecord") {
			invoiceId = `INV-${Math.floor(1e3 + Math.random() * 9e3)}`;
			...
			await supabase.from("orders").insert({
				...
				status: "processing",
				placedAt: "Just now",
				aiNote: "Restructured pipeline order."
			});
		...
		if (step.name === "generateInvoice") await supabase.from("invoices").insert({
			id: crypto.randomUUID(),
		...
		if (step.name === "updateDues") {
			const { data: dl } = await supabase.from("dealers").select("pending, ordersCount").eq("id", context.dealerId).maybeSingle();
		...
		if (step.name === "recordPayment") {
			const paidAmount = context.changes?.[0]?.amount || 2e4;
			const { data: promises } = await supabase.from("payment_promises").select("*").eq("dealer_id", context.dealerId).eq("status", "pending");
			let matchedPromiseId = null;
			for (const p of promises || []) {
				const pDate = new Date(p.promised_date);
				const diffDays = Math.abs((/* @__PURE__ */ new Date()).getTime() - pDate.getTime()) / (1e3 * 60 * 60 * 24);
				const amtMatch = paidAmount >= p.promised_amount * .9;
				if (diffDays <= 5 && amtMatch) {
					matchedPromiseId = p.id;
					break;
				}
			}
			if (matchedPromiseId) await supabase.from("payment_promises").update({ status: "kept" }).eq("id", matchedPromiseId);
			const { data: dl } = await supabase.from("dealers").select("pending").eq("id", context.dealerId).maybeSingle();
			const beforeAmount = dl?.pending || 0;
			const remaining = Math.max(0, beforeAmount - paidAmount);
			await supabase.from("dealers").update({ pending: remaining }).eq("id", context.dealerId);
		}
		if (step.name === "updateLedger") {}
		if (step.name === "recalculateTrustScore") await adjustTrustScore(context.dealerId);
		if (step.name === "fetchLastOrder") {
			const { data: dbOrders } = await supabase.from("orders").select("*").eq("dealerId", context.dealerId).order("created_at", { ascending: false });
			const orders = dbOrders || [];
			if (orders.length === 0) throw { type: "NO_ORDER_HISTORY" };
			const userMsgText = context.text || "";
			if ((userMsgText.toLowerCase().includes("usual") || userMsgText.toLowerCase().includes("typical")) && orders.length > 1) {
				if (Array.from(new Set(orders.map((o) => o.total))).length > 1) throw {
					type: "AMBIGUOUS_USUAL_ORDERS",
					orders: orders.slice(0, 3)
				};
			}
			const targetOrder = orders[0];
			const { data: dbOrderItems } = await supabase.from("order_items").select("*").eq("orderId", targetOrder.id);
			const orderItems = dbOrderItems || [];
			if (orderItems.length === 0) throw new Error(`Order ${targetOrder.invoice} has no items.`);
			const { data: dbProducts } = await supabase.from("products").select("*");
			const products = dbProducts || [];
			items = orderItems.map((oi) => {
				const match = products.find((p) => p.name.toLowerCase() === oi.name.toLowerCase()) || products.find((p) => p.sku.toLowerCase() === oi.name.toLowerCase()) || products.find((p) => p.name.toLowerCase().includes(oi.name.toLowerCase()));
				return {
					sku: match?.sku || "MCB-32A-SP",
					name: match?.name || oi.name,
					qty: oi.qty,
					price: match?.price || oi.price
				};
			});
		}
		if (step.name === "updateDuesRecord") {
			const promisedDate = parsePromisedDate(context.changes?.[0]?.reminderWhen);
			const promisedAmount = context.changes?.[0]?.paymentAmount || 2e4;
...
async function processAgentRequest(text, conversationId, dealerName) {
	...
	if (!(await validateRequest(text, dealerId)).valid) {
		transitionTo("FAILED");
		const elapsed = Date.now() - startTime;
		let politeReply = "Hello sir, let me know how I can assist with outstanding dues or stock checks.";
		const responseObj = {
			traceId,
			state: "FAILED",
			intent: "BUSINESS_QUERY",
	...
	const state = await getConversationState(conversationId, dealerId);
	await getDealerProfile(dealerId);
	transitionTo("THINKING");
	const classification = await classifyIntent(text, state.last_draft, state.last_intent);
	const { intent, confidence, subIntent, extractedItems, extractedChanges, paymentAmount, reminderWhen, reminderNote } = classification;
	if (confidence < .5) {
		transitionTo("FAILED");
		const elapsed = Date.now() - startTime;
		const failedReply = "I could not understand that request clearly. Could you please check the model name or dues value?";
		const responseObj = {
			traceId,
			state: "FAILED",
			intent,
			...
			health: "ERROR",
			guardrails: "PASSED",
			plan: ["Reject due to low confidence"],
			timeline,
			toolsUsed: [],
			reflection: {
				success: false,
				summary: "Halted due to confidence threshold < 0.5"
			},
			executionTime: `${elapsed}ms`,
	...
	if (confidence < .8) {
		...
		const elapsed = Date.now() - startTime;
		const reply = classification.response || "Could you please clarify exactly which product code or quantity you need?";
		const responseObj = {
			traceId,
	...
	try {
		if (intent === "ORDER" || intent === "REPEAT_ORDER") {
			if (intent === "REPEAT_ORDER" || !state.last_draft || subIntent === "new_draft") {
				const plan = getPlan(intent === "REPEAT_ORDER" ? "REPEAT_ORDER" : "ORDER", true);
				toolsUsed.push(...plan.map((s) => s.name));
				const result = await executePlan(plan, {
					conversationId,
					dealerId,
					dealerName,
					items: intent === "REPEAT_ORDER" ? [] : extractedItems,
					text
				}, state.last_draft);
				...
				kind = "order";
				responseText = `Understood sir! I have drafted your order for ${result.items.map((i) => `**${i.qty}x ${i.name}**`).join(" and ")} (Total: **₹${result.total.toLocaleString("en-IN")}**). Please reply with 'Confirm' to finalize the order and generate your invoice.`;
				dataPayload = {
					title: intent === "REPEAT_ORDER" ? "Repeat Order Draft" : "Order Draft",
					delivery: "Standard Delivery",
					total: result.total,
					items: result.items
				};
			} else if (subIntent === "correction") {
				const updated = await recalculateDraft(state.last_draft, extractedChanges);
				await updateConversationState(conversationId, {
					last_draft: {
						items: updated.items,
						total: updated.total
				...
				kind = "order";
				responseText = `Understood sir! I have corrected your order draft: ${updated.items.map((i) => `**${i.qty}x ${i.name}**`).join(" and ")} (Total: **₹${updated.total.toLocaleString("en-IN")}**). Please reply with 'Confirm' to finalize.`;
				dataPayload = {
					title: "Order Draft (Updated)",
					delivery: "Standard Delivery",
					total: updated.total,
					items: updated.items
				};
			} else if (subIntent === "confirm") {
				if (checkStaleness(state.last_activity_at, state.turn_count, state.last_intent)) {
					transitionTo("WAITING_FOR_USER");
					const elapsed = Date.now() - startTime;
					const staleReply = `Are you confirming the order for ${state.last_draft.items.map((i) => `**${i.qty}x ${i.name}**`).join(" and ")} from earlier, or something else?`;
					await updateConversationState(conversationId, {
						last_activity_at: (/* @__PURE__ */ new Date()).toISOString(),
						turn_count: state.turn_count + 1
					});
					const responseObj = {
						...
						health: "WARNING",
						guardrails: "PASSED",
						plan: ["Clarify stale draft confirmation"],
						timeline,
						toolsUsed: [],
						reflection: {
							success: true,
							summary: "Gated confirmation due to staleness rules."
						},
						executionTime: `${elapsed}ms`,
						response: staleReply,
						text: staleReply,
				...
				const plan = getPlan("ORDER", false);
				toolsUsed.push(...plan.map((s) => s.name));
				const result = await executePlan(plan, {
					conversationId,
					dealerId,
				...
				const verified = await reflectAndVerify(result);
				reflectionResult = {
					success: verified.consistent,
				...
				if (!verified.consistent) {
					...
					const responseObj = {
						...
						health: "ERROR",
						guardrails: "PASSED",
						plan: plan.map((s) => s.purpose),
						timeline,
						toolsUsed,
						reflection: reflectionResult,
						executionTime: `${elapsed}ms`,
						response: "Sir, there was a problem saving your order record. Stock has not been deducted. Please retry.",
						text: "Sir, there was a problem saving your order record. Stock has not been deducted. Please retry.",
						kind: "text",
						data: null
					};
					return JSON.stringify(responseObj);
				...
				kind = "invoice";
				responseText = `Outstanding sir! I have finalized your order and generated your invoice receipt: **Invoice ${result.invoiceId}** (Total: **₹${result.total.toLocaleString("en-IN")}**). Stocks have been updated.`;
				dataPayload = {
			...
			const amount = paymentAmount || 2e4;
			const plan = getPlan("PAYMENT", false);
			toolsUsed.push(...plan.map((s) => s.name));
			...
			kind = "ledger";
			responseText = `Thank you sir! Received **₹${amount.toLocaleString("en-IN")}** via UPI. Your outstanding balance has been updated to **₹${remaining.toLocaleString("en-IN")}** in our ledger.`;
			dataPayload = {
				paid: amount,
				remaining,
				before: remaining + amount
			};
		} else if (intent === "PAYMENT_PROMISE") {
			const amount = paymentAmount;
			if (!amount) {
				transitionTo("WAITING_FOR_USER");
				const elapsed = Date.now() - startTime;
				const clarifyAmtText = "Sir, could you please clarify the promised payment amount (is it the full outstanding amount or a partial payment, and how much)?";
				const responseObj = {
					traceId,
					state: "WAITING_FOR_USER",
					intent,
					...
					health: "WARNING",
					guardrails: "PASSED",
					plan: ["Clarify promised amount"],
					timeline,
					toolsUsed: [],
					reflection: {
						success: true,
						summary: "Clarification needed on promised amount."
					},
					executionTime: `${elapsed}ms`,
					response: clarifyAmtText,
					text: clarifyAmtText,
					kind: "text",
					data: null
				};
			...
			const plan = getPlan("PAYMENT_PROMISE", false);
			toolsUsed.push(...plan.map((s) => s.name));
			const when = reminderWhen || "Nov 5, 10:00 AM";
			const note = reminderNote || "Dues payment follow up";
			await executePlan(plan, {
				conversationId,
				dealerId,
				dealerName,
			...
			await updateConversationState(conversationId, {
				last_intent: "PAYMENT_PROMISE",
				last_activity_at: (/* @__PURE__ */ new Date()).toISOString(),
				turn_count: state.turn_count + 1
			});
			kind = "reminder";
			responseText = `Understood sir. I have registered your payment promise for **${when}**. A gentle collection follow-up has been scheduled.`;
			dataPayload = {
				when,
				note,
				dueDate: calculateDueDate(when)
			};
		} else {
			await updateConversationState(conversationId, {
				last_intent: intent,
			...
			kind = "text";
			responseText = classification.response || "Hello sir, please let me know how I can help you verify dues or check stock levels.";
		}
	} catch (err) {
		if (err && err.type === "AMBIGUOUS_SKU") {
			if (err.validatedItems && err.validatedItems.length > 0) await updateConversationState(conversationId, {
				last_draft: {
					items: err.validatedItems,
					total: err.validatedItems.reduce((s, i) => s + i.qty * i.price, 0)
				},
				last_intent: "ORDER",
				last_activity_at: (/* @__PURE__ */ new Date()).toISOString(),
				turn_count: state.turn_count + 1
			...
			let clarifyText = "";
			if (err.validatedItems && err.validatedItems.length > 0) clarifyText += `Drafted ${err.validatedItems.map((i) => `**${i.qty}x ${i.name}**`).join(" and ")}. `;
			const amb = err.ambiguousMatches[0];
			const options = amb.matches.map((m) => `• **${m.name}** (SKU: \`${m.sku}\`, ₹${m.price})`).join("\n");
			clarifyText += `Sir, '${amb.item.sku || amb.item.name}' matches multiple products. Did you mean:\n${options}\nPlease clarify which SKU you need.`;
			transitionTo("WAITING_FOR_USER");
			const responseObj = {
				traceId,
				state: "WAITING_FOR_USER",
				...
				health: "WARNING",
				guardrails: "PASSED",
				plan: ["Clarify ambiguous SKU"],
				timeline,
				toolsUsed: [],
				reflection: {
					success: true,
					summary: "SKU ambiguity encountered."
				},
			...
			return JSON.stringify(responseObj);
		} else if (err && err.type === "INSUFFICIENT_STOCK") {
			if (err.inStockItems && err.inStockItems.length > 0) await updateConversationState(conversationId, {
				last_draft: {
					items: err.inStockItems,
					total: err.inStockItems.reduce((s, i) => s + i.qty * i.price, 0)
				},
				last_intent: "ORDER",
				last_activity_at: (/* @__PURE__ */ new Date()).toISOString(),
				turn_count: state.turn_count + 1
			...
			let responseText = "";
			if (err.inStockItems && err.inStockItems.length > 0) responseText += `Drafted ${err.inStockItems.map((i) => `**${i.qty}x ${i.name}**`).join(" and ")}. `;
			const oos = err.outOfStockItems[0];
			responseText += `We only have ${oos.available} units of '${oos.item.name}' available (requested ${oos.item.qty}). Would you like to order ${oos.available} now, wait for restock, or cancel/change this item?`;
			transitionTo("WAITING_FOR_USER");
			const responseObj = {
				traceId,
				state: "WAITING_FOR_USER",
				...
				health: "WARNING",
				guardrails: "PASSED",
				plan: ["Handle inventory shortage"],
				timeline,
				toolsUsed: [],
				reflection: {
					success: true,
					summary: "Insufficient stock encountered."
				},
				executionTime: `${Date.now() - startTime}ms`,
			...
			return JSON.stringify(responseObj);
		} else if (err && err.type === "NO_ORDER_HISTORY") {
			transitionTo("FAILED");
			const elapsed = Date.now() - startTime;
			const noHistoryReply = "Sir, I could not find any order history for your account. What items would you like to order today?";
			const responseObj = {
				traceId,
				state: "FAILED",
				intent: "ORDER",
				...
				health: "WARNING",
				guardrails: "PASSED",
				plan: ["Reject due to no order history"],
				timeline,
				toolsUsed: [],
				reflection: {
					success: true,
					summary: "No past orders found."
				},
				executionTime: `${elapsed}ms`,
				response: noHistoryReply,
				text: noHistoryReply,
				kind: "text",
				data: null
			};
			return JSON.stringify(responseObj);
		} else if (err && err.type === "AMBIGUOUS_USUAL_ORDERS") {
			transitionTo("WAITING_FOR_USER");
			const elapsed = Date.now() - startTime;
			const ambiguousUsualReply = `Sir, you have multiple different recent orders. Would you like to repeat order ${err.orders.map((o) => `${o.invoice} (₹${o.total.toLocaleString("en-IN")})`).join(" or ")}?`;
			const responseObj = {
				traceId,
				state: "WAITING_FOR_USER",
				intent: "ORDER",
				...
				health: "WARNING",
				guardrails: "PASSED",
				plan: ["Clarify ambiguous usual orders"],
				timeline,
				toolsUsed: [],
				reflection: {
					success: true,
					summary: "Ambiguous repeat orders."
				},
				executionTime: `${elapsed}ms`,
				response: ambiguousUsualReply,
				text: ambiguousUsualReply,
				kind: "text",
				data: null
			};
			return JSON.stringify(responseObj);
	...
	logAgentExecution({
		...
		toolSuccessCount: toolsUsed.length,
		toolFailureCount: 0,
		health: "HEALTHY"
	});
...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/db-queries-BImAPcmW.js
...
async function runAgentConversation(conversationId, dealerName, chatHistory, currentMessage) {
	return await processAgentRequest(currentMessage || chatHistory[chatHistory.length - 1]?.parts[0]?.text || "", conversationId, dealerName);
}
...
[
  {
    ...
    "promise": "string or null"
		...
		try {
			...
			if (response.ok) {
				const jsonStr = ((await response.json()).choices?.[0]?.message?.content || "").trim().replace(/^```json/, "").replace(/```$/, "").trim();
				return JSON.parse(jsonStr);
			} else console.error("Groq completions request failed with status:", response.status);
		} catch (err) {
	...
	return dealersList.map((d) => {
		...
		if (nameStr.includes("Verma") || nameStr.includes("Vijay") || nameStr.includes("Raj Traders")) promise = "Nov 5 (Post-Diwali)";
		return {
			dealerId: d.id,
			risk,
			action,
...
var getDashboardData = createServerFn({ method: "GET" }).handler(getDashboardData_createServerFn_handler, async () => {
	const [{ data: dealers }, { data: products }, { data: orders }, { data: invoices }, { count: reminderCount }] = await Promise.all([
...
var getDealerById = createServerFn({ method: "GET" }).validator((id) => id).handler(getDealerById_createServerFn_handler, async ({ data: id }) => {
	...
	const [{ data: orders }, { data: invoices }, { data: conversations }] = await Promise.all([
	...
	if (activeConvo) messages = (activeConvo.messages || []).map((m) => ({
		id: String(m.id),
...
var getOrders = createServerFn({ method: "GET" }).handler(getOrders_createServerFn_handler, async () => {
	const { data: orders } = await supabase.from("orders").select("*, order_items(name, qty, price)").order("invoice", { ascending: false });
...
var getConversationsList = createServerFn({ method: "GET" }).handler(getConversationsList_createServerFn_handler, async () => {
	...
	return (conversations || []).filter((c) => c.preview !== "Seeded for regression test" && !String(c.id).startsWith("baseline-convo-")).map((c) => {
		const msgs = c.messages || [];
		const parsedMessages = msgs.filter((m) => m.fromRole !== "system_memory").map((m) => ({
			id: String(m.id),
			from: String(m.fromRole),
			text: m.text ? String(m.text) : void 0,
			time: String(m.time),
		...
		parsedMessages.sort((a, b) => {
			...
			if (isSeededA && isSeededB) return parseInt(a.id.substring(1)) - parseInt(b.id.substring(1));
...
var postMessage = createServerFn({ method: "POST" }).validator((data) => data).handler(postMessage_createServerFn_handler, async ({ data }) => {
	...
	const aiTime = (/* @__PURE__ */ new Date()).toISOString();
	await supabase.from("messages").insert({
		id: aiMsgId,
		conversationId: data.conversationId,
		fromRole: "ai",
...
async function runCronLogic(forceAll = false) {
	const now = /* @__PURE__ */ new Date();
	const todayStr = now.toISOString().split("T")[0];
	const { data: overduePromises } = await supabase.from("payment_promises").select("*").eq("status", "pending");
	for (const p of overduePromises || []) if (p.promised_date < todayStr) {
		await supabase.from("payment_promises").update({ status: "broken" }).eq("id", p.id);
		await adjustTrustScore(p.dealer_id);
	}
	const { data: reminderMsgs, error } = await supabase.from("messages").select("*").eq("kind", "reminder");
	if (error) {
	...
	for (const msg of reminderMsgs || []) try {
		...
		if (!payload) continue;
		if ((payload.status || "pending") !== "pending") continue;
		const dueDateStr = payload.dueDate;
		if (!dueDateStr && !forceAll) continue;
		if (dueDateStr && !forceAll) {
			if (new Date(dueDateStr) > now) continue;
		}
		payload.status = "processing";
		await supabase.from("messages").update({ data: JSON.stringify(payload) }).eq("id", msg.id);
		...
		payload.status = "sent";
		payload.sentAt = (/* @__PURE__ */ new Date()).toISOString();
		await supabase.from("messages").update({ data: JSON.stringify(payload) }).eq("id", msg.id);
		processedCount++;
	} catch (itemErr) {
...
var runCronAction_createServerFn_handler = createServerRpc({
	id: "e439ec9d9e66247d12dd0c22009c6affdbfd695d8b1bbf061f8e8981af9ab510",
	name: "runCronAction",
	filename: "src/lib/db-queries.ts"
}, (opts) => runCronAction.__executeServer(opts));
var runCronAction = createServerFn({ method: "POST" }).validator((data) => data).handler(runCronAction_createServerFn_handler, async ({ data }) => {
	try {
		return await runCronLogic(data?.forceAll ?? false);
	} catch (err) {
...
var getDues = createServerFn({ method: "GET" }).handler(getDues_createServerFn_handler, async () => {
	const { data: dealersData } = await supabase.from("dealers").select("*").gt("pending", 0).order("pending", { ascending: false });
	const dealersList = dealersData || [];
	const { data: promisesData } = await supabase.from("payment_promises").select("*").eq("status", "pending");
	const promisesList = promisesData || [];
	return dealersList.map((d) => {
		const overdueDays = d.status === "overdue" ? 42 : d.status === "watch" ? 18 : 0;
		const dPromise = promisesList.find((p) => p.dealer_id === d.id);
		const promise = dPromise ? dPromise.promised_date : null;
		let risk = 12;
		let action = "Auto-reminder scheduled";
		if (d.status === "overdue") {
			risk = 85;
...
var clearConversationAction_createServerFn_handler = createServerRpc({
	id: "92ebcdec2baf99b61f9d65871c39a3f5b673c57e0973dba64de1f9e9701c0389",
	name: "clearConversationAction",
	filename: "src/lib/db-queries.ts"
}, (opts) => clearConversationAction.__executeServer(opts));
var clearConversationAction = createServerFn({ method: "POST" }).validator((data) => data).handler(clearConversationAction_createServerFn_handler, async ({ data }) => {
...
//#endregion
export { askAiQuery_createServerFn_handler, clearConversationAction_createServerFn_handler, confirmOrderAction_createServerFn_handler, createProductAction_createServerFn_handler, getAiDuesAnalysis_createServerFn_handler, getConversationsList_createServerFn_handler, getDashboardData_createServerFn_handler, getDealerById_createServerFn_handler, getDealers_createServerFn_handler, getDues_createServerFn_handler, getInventory_createServerFn_handler, getInvoiceDetailsAction_createServerFn_handler, getInvoices_createServerFn_handler, getOrders_createServerFn_handler, postMessage_createServerFn_handler, recordPaymentAction_createServerFn_handler, runCronAction_createServerFn_handler, updateOrderStatusAction_createServerFn_handler };
